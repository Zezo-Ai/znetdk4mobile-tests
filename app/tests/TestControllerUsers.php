...
namespace app\tests;
use app\TestCase;
class TestControllerUsers extends TestCase {

    static protected function isActionAllowed() {
        $menuItem = CFG_PAGE_LAYOUT === 'mobile' ? 'z4musers' : 'users';
        // TEST 1: auth enabled, test user has access to user menu item: controller's action is executed
        ...
        $test1 = is_array($response) && $response['user_id'] === $credentials['id'];

        // TEST 2: auth enabled, test if user has no access to the user menu item: HTTP error 403
        ...
        // TEST 3: no auth, users view menu item is declared in the menu.php
        $extraParameters3 = [
            'id' => $credentials2['id']/*,
            'XDEBUG_SESSION_START' => 'netbeans-xdebug' */
        ];
        ...
        $test3 = is_array($response3) && $response3['user_id'] === $credentials2['id'];
